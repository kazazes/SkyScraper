      ...
      <v-card class="card">
        <v-card-title primary-title class="red darken-1 white--text py-1 align-baseline">
          <h4
            class="subtitle mt-2"
          >
            {{ selected && selected.talkgroup ? selected.talkgroup.description : "Loading..." }}
          </h4>
        </v-card-title>
        ...
        <v-card-actions class="mx-0 px-0 pb-0">
          <Player
            ...
            :file="fileSource"
            @play-live-audio="$emit('play-live-audio')"
            @play-next-audio="$emit('play-next-audio')"
            @player-state-pause="$emit('player-state-pause')"
            @player-state-ended="$emit('player-state-ended')"
          />
        </v-card-actions>
      </v-card>
    </v-flex>
      ...
      <v-card class="card">
        ...
        <v-card-text class="white">
          <client-only>
            ...
            />
            <v-flex v-else justify-space-around>
              <v-progress-circular style="width: 100%" color="primary" indeterminate />
            </v-flex>
          </client-only>
        </v-card-text>
      </v-card>
